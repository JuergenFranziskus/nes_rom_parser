...
impl<'a> Rom<'a> {
    pub fn parse(src: &'a [u8]) -> Result<Self, RomErr> {
        ...
        let trainer = &src[trainer_start..trainer_end];
        ...
        let prg_rom = &src[prg_rom_start..prg_rom_end];
        ...
        let chr_rom = &src[chr_rom_start..chr_rom_end];

        let msc_rom = &src[chr_rom_end..];
